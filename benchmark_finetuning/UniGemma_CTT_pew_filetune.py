...
parser.add_argument('--benchmark-path', type=str, default = f"{base_dir}/data/pew_data.json", help='Path to the input json file')
...
parser.add_argument('--output-path', type=str, default = "/home/msm97/scratch/model_checkpoints/CTT/pew/2", help='Path to the output folder')
parser.add_argument('--images-path', type=str, default=f'{base_dir}/data/pew_images', help='Path to the images Folder')
...
parser.add_argument('--checkpoint-steps', type=int, default=1400, help='Steps needed to record a checkpoint')
...
class MultimodalModelPL(pl.LightningModule):
    ...
    def val_dataloader(self):
        return DataLoader(self.val_dataset, self.config["val_batch_sizes"], shuffle=False, num_workers=4)
...
huh_ev = DonutDataset(args.benchmark_path, args.images_path, args.context_length, processor, split = 'val', prompt_end_token = '</ins>')#, indices = list(range(128)))
