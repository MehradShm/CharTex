...
#        task_start_token: str = "<s>",
        ...
        #dataset_dict = load_dataset(dataset_name_or_path, split=f"train[{sw}:{ew}]")
        # self.dataset = dataset_dict[split]
        dataset_dict = load_dataset(dataset_name_or_path)
        dataset_dict = dataset_dict[split].select(indices)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        processed_parse = sample['query'] + " " + self.prompt_end_token + " " + sample['label'] + self.processor.tokenizer.eos_token #random.choice(self.gt_token_sequences[idx])  # can be more than one, e.g., DocVQA Task 1
        ...
        if self.split == "train":
            ...
            tmp = {"pixel_values": input_tensor, "input_ids": input_ids, "labels": labels}
